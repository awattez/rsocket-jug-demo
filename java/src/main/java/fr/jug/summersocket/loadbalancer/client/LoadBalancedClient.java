package fr.jug.summersocket.loadbalancer.client;
...
import static fr.jug.summersocket.loadbalancer.server.MultipleServers.HOST;
...
public class LoadBalancedClient {
    ...
    public static void main(String[] args) {
        ...
        List<RSocketSupplier> rsocketSuppliers = Arrays.stream(PORTS)
                .mapToObj(port -> new RSocketSupplier(() -> Mono.just(RSocketConnector
                        .connectWith(TcpClientTransport.create(HOST, port)).block()
                )))
